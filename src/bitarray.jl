...
"""
    BitView{N} <: AbstractArray{Bool, N}
...
mutable struct BitView{T, N} <: AbstractArray{Bool, N}
    chunks::Vector{UInt64}
    len::Int
    dims::NTuple{N,Int}
    function BitView{T, N}(a::Array{T, N}) where {T, N}
    	dims = sizeof(T).*(size(a))
        nc = div(prod(dims), 64, RoundUp)
        chunks = Vector{UInt64}(undef, nc)
        nc > 0 && (chunks[end] = UInt64(0))
        b = new(chunks, nc, dims)
        N != 1 && (b.dims = dims)
        return b
    end
end

mutable struct A{T, N}
	ch::Vector{T}
	dims::NTuple{N, Int}
	function A(a::Array{T, N}) where {T, N}
		r = new{T, N}(a[:], size(a))
		return r
	end
end
...
# to fix this.
"""
    BitView(undef, dims::Integer...)
    BitView{N}(undef, dims::NTuple{N,Int})

Construct an undef [`BitView`](@ref) with the given dimensions.
Behaves identically to the [`Array`](@ref) constructor. See [`undef`](@ref).

# Examples
```julia-repl
julia> BitView(undef, 2, 2)
2×2 BitMatrix:
 0  0
 0  0

julia> BitView(undef, (3, 1))
3×1 BitMatrix:
 0
 0
 0
...
BitView(a::Array{T, N}, dims::Integer...) where {T, N} = BitView{T, N}(a, map(Int,dims))
BitView(a::Array{T, N}) where {T, N} = BitView{T, N}(a)
Base.size(::BitView{T, N}) where {T, N} = sizeof(UInt64).*size(chunks)
